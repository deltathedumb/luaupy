...
class LuauBackend(Backend):
    ...
    def emit(self, module: Module, target: Target) -> dict[str, bytes]:
        ...
        if self.layout == "flat":
            # One file: the runtime inlined ahead of the program, with the
            # `require` dropped. For pasting straight into a Script.
            joined = body.replace(_REQUIRE_LINE + "\n", "")
            return {"init.luau": (rt + "\n\n" + _FLAT_BRIDGE + "\n\n"
                                  + joined).encode("utf-8")}
...
#: In the flat layout the runtime's own `return` has to become a local instead.
_FLAT_BRIDGE = "local RT = _RUNTIME"
